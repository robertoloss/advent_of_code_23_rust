//WARNING! This code is in bad need of some *serious* refactoring!


fn create_vec_of_strings(input: String) -> Vec<String> {
    let vec_of_strings = input
        .split('\n')
        .map(|s| s.to_string())
        .collect();
...
fn generate_matrix(vec_of_strings: Vec<String>) -> Vec<Vec<char>> {
    let mut matrix : Vec<Vec<char>> = vec![];
    for s in vec_of_strings.iter() {
...
fn generate_vec_of_part_numbers(matrix: &Vec<Vec<char>>) -> Vec<i32> {
    ...
    while y < num_of_rows {
        while x < num_of_cols {
            if matrix[y][x].is_numeric() {
                ...
                while z < num_of_cols && matrix[y][z].is_numeric() {
                    number.push(matrix[y][z]);
                    z += 1;
                }
                ...
                let right = if end == num_of_cols-1 {
                    end 
                } else {
                    end+1
                };
                ...
                let bottom = if y == num_of_rows-1 {
                    y 
                } else { 
                    y+1
                };
                //println!("Left, right, top, bottom for {} are : {}, {}, {}, {}", 
                         // number, left, right, top, bottom);

                'outer: for i in top..bottom+1 {
                    'inner: for j in left..right+1 {
                        // println!("matrix[{}][{}] for {} is {}", 
                                 //i, j, number, matrix[i][j]);
...
fn generate_vec_of_gear_ratios(matrix: Vec<Vec<char>>, vec_of_part_numbers: Vec<i32>) -> Vec<i32> {
    let num_of_rows = matrix.len();
    let num_of_cols = matrix[0].len();
    let mut x = 0;
    let mut y = 0;
    ...
    while y < num_of_rows {
        while x < num_of_cols {
            if matrix[y][x] == '*' {
                //println!("I have found a gear! Its coordinates are {}:{}", y,x);
                let left = if x > 0 {
                    x-1 
                } else {
                    0
                };
                let right = if x == num_of_cols-1 {
                    x
                } else {
                    x+1
                };
                ...
                while i < bottom+1 {
                    while j < right+1 {
                        if matrix[i][j].is_numeric() {
                            ...
                            while   z as i32 >= 0 && 
                                    matrix[i][z].is_numeric() && 
                                    z_negative == false 
                            {
                                if z > 0 { z -= 1 } else { z_negative = true }
                            }
                            z = if z_negative == true { 0 } else { z+1 };


                            let mut number = String::from("");
                            while z < num_of_cols && matrix[i][z].is_numeric() {
                                number.push(matrix[i][z]);
                                z += 1;
                            }
                            //println!("Found a number: {}", number);
                            let num = number.parse::<i32>().unwrap();
                            if vec_of_part_numbers.contains(&num) {
                                //println!("{} is a part number", num);
                                gears.push(num);
                            }
                            j = z;
                        }
...
fn main() {
    let input = String::from("48.................501....33.....622..............763.........331.................161.683......................................980..........
...
...........836......383...........557.....672..........................764.....944............................827...........................");
    ...
    let vec_of_strings = create_vec_of_strings(input);

    let matrix = generate_matrix(vec_of_strings);

    let vec_of_part_numbers = generate_vec_of_part_numbers(&matrix);

    let vec_of_gear_ratios = generate_vec_of_gear_ratios(matrix, vec_of_part_numbers);

    let result = vec_of_gear_ratios 
        .iter()
        .fold(0, |acc, &x| acc + x);
