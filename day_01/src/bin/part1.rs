...
fn find_calibration(line : &String) -> i32 {
    ...
    while left <= right  && (left_char.is_none() || right_char.is_none()) {
       let (c_left,c_right) =  (
            line.chars().nth(left).unwrap(),
            line.chars().nth(right).unwrap()
        );
       if left_char.is_none() {
           if c_left.is_numeric() {
    ...
    result_string.push(left_char.unwrap());
    result_string.push(right_char.unwrap());
...
fn main() {
    let example = String::from("a1b2c3d4e5f");
    println!("{}", example);
