...
fn find_calibration(line : &String) -> i32 {
    ...
    let mut left_string : String = String::from("");
    let mut right_string : String = String::from("");
    ...
    while left <= right  {
        ...
        let (c_left,c_right) =  (
            line.chars().nth(left).unwrap(),
            line.chars().nth(right).unwrap()
        );
        if left_char.is_none() {
            for word in words.iter() {
                if left_string.len() > 2 && left_string.contains(word) {
                    let num = match word {
                        &"one" => 1,
                        &"two" => 2,
                        &"three" => 3,
                    ...
                    println!("\nLEFT: found the word {} in {}", word, left_string);
                    let st = std::char::from_digit(num as u32, 10);
                    left_char = st;
                    break;
                }
            }
            if c_left.is_numeric() && left_char.is_none() {
                left_char = Some(c_left);
                println!("\nLEFT: Found the number {}", c_left);
           } else if left_char.is_none() { 
                left += 1;
                left_string.push(c_left);
                if left_string.len() > 5 {
                    left_string = left_string[1..left_string.len()].to_string();
                }
        ...
        if right_char.is_none() {
            for word in words.iter() {
                if right_string.len() > 2 && right_string.contains(word) {
                    let num = match word {
                        &"one" => 1,
                        &"two" => 2,
                        &"three" => 3,
                    ...
                    println!("RIGHT: found the word {} in {}", word, right_string);
                    let st = std::char::from_digit(num as u32, 10);
                    right_char = st;
                    break;
                }
            }
            if c_right.is_numeric() && right_char.is_none() {
                right_char = Some(c_right);
                println!("RIGHT: Found the number {}", c_right);
            } else if right_char.is_none() {
                right -= 1;
                right_string.insert(0, c_right);
                if right_string.len() > 5 {
                    right_string = right_string[0..right_string.len()-1].to_string(); 
                }
            }
        } 
    }

    let result_string = format!("{}{}", left_char.unwrap(), right_char.unwrap());
    let result_num = result_string.parse::<i32>().unwrap_or(-32);

    return result_num;
}

fn create_vec(blob: &String) -> Vec<String> {
    blob
        .split('\n')
        .map(|l| l.to_string())
        .collect()
...
fn main() {
    //let example = String::from("abcone2threexyz");
    //println!("{}", example);
    //println!("{}", find_calibration(&example));

    let blob = String::from("nine92jnhgqzctpgbcbpz
sevensddvc73three
9986fmfqhdmq8
7onexmxbzllfqb
six777
...
9one9pjtnncsqzhcszp5");
    ...
    let vec_from_blob = create_vec(&blob);

    for i in vec_from_blob.iter() {
        println!("{}", i)
    }

    let sum = vec_from_blob
        .iter()
        .map(| line | find_calibration(&line))
