...
fn create_list_of_games(input: String) -> Vec<String> {
    let result = input
        .split('\n')
        .map(|s| s.to_string())
        .collect();
...
fn check_if_game_is_possible(
    input: String,
    max_red: i32,
    max_green: i32,
    max_blue: i32) -> bool {

    let arr_of_str : Vec<&str> = input
        .split(':')
        .nth(1)
        .unwrap()
        .split(';')
    ...
    for i in arr_of_str.iter() {
        println!("{}", i);
    }
    
    let mut vec_of_hash_maps : Vec<HashMap<&str,usize>> = vec![];

    for i in arr_of_str.iter() {
        let mut hash_map = HashMap::<&str,usize>::new();
        for j in i.split(',') {
            let num_color: Vec<&str> = j.split_whitespace().collect();
            let color = num_color[1];
            let number = num_color[0];
            let num = number.parse::<usize>().unwrap();   
            hash_map.insert(color, num);   
        }
        vec_of_hash_maps.push(hash_map);
    }
    let mut counter = 0;
    for hm in vec_of_hash_maps.iter() {
        //println!("");
        //println!("red : {:?}", hm.get("red"));
        //println!("green : {:?}", hm.get("green"));
        //println!("blue : {:?}", hm.get("blue"));
        
        let r = *hm.get("red").unwrap_or(&0);
        let g = *hm.get("green").unwrap_or(&0);
        let b = *hm.get("blue").unwrap_or(&0);

        if  r > (max_red as usize) ||
            g > (max_green as usize) ||
            b > (max_blue as usize) {
            break
        }
        counter += 1;
    }
    if counter == vec_of_hash_maps.len() {
        println!("This game is possible");
    }

    return counter==vec_of_hash_maps.len()
}


fn main() {
    let input = String::from("Game 1: 12 blue; 2 green, 13 blue, 19 red; 13 red, 3 green, 14 blue
...
Game 100: 8 red, 4 blue, 4 green; 10 blue, 3 red, 4 green; 10 green, 4 red; 18 red, 9 blue, 2 green; 12 red, 4 green, 2 blue");
    ...
    let mut list_of_possible_games : Vec<i32> = vec![];

    let list_of_games : Vec<String> = create_list_of_games(input);
    
    for i in 0..list_of_games.len() {
        if check_if_game_is_possible
            (
                list_of_games[i].to_string(),
                max_red,
                max_green,
                max_blue
            ) {
            list_of_possible_games.push((i+1) as i32);
        }
    }

    let result = list_of_possible_games.iter().fold(0, |acc, &x| acc + x);
